...
const SectionsPage = ({ data: { kontentItemSectionsPage: pageData } }) => {
  ...
  const sections = pageData.elements.sections.value.map((section, index) => {
    const image = section.elements.image.value.length > 0
      ? <a href={section.elements.cta.value[0].elements.external_url.value}
        className="image">
        <img src={section.elements.image.value[0].url} alt={section.elements.image.value[0].title} />
      </a>
      : null;
    const actions = section.elements.cta.value.length > 0
      ? section.elements.cta.value.map(cta => (
        <li key={cta.elements.external_url.value}>
          <a href={cta.elements.external_url.value}
            className="button">{cta.elements.title.value}</a>
        </li>))
      : null;
      ...
      <section className={image && "spotlights"}>
        ...
        <div className="content">
          <div className="inner">
            ...
            <p dangerouslySetInnerHTML={{ __html: section.elements.content.value }} />
